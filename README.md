...
- **JWT Tokens**: Access tokens expire after a configured time
- **Password Hashing**: Uses bcrypt with proper salt rounds
- **CORS**: Configure `allow_origins` in production for your domain
- **Environment Variables**: Never commit `.env` file to version control
- **Secret Key**: Use a strong, random secret key in production (32+ characters)

## Troubleshooting

### Common Issues

**Issue: Cannot connect to Google API**
- Verify your `GOOGLE_API_KEY` is correct in `.env`
- Check your internet connection
- Ensure you have API quota remaining

**Issue: YOLO model not loading**
- Ensure `yolo11m.pt` is in the project root
- The file will auto-download on first use if missing
...
- **Subsequent Requests**: Optimized with model caching
...
- [ ] Multiple AI model options (YOLOv8, Detectron2, etc.)
- [ ] Advanced RAG with vector database for image history
